...
define('SMARTIRC_VERSION', '1.1.0-dev ($Revision: 1.90 $)');
...
class Net_SmartIRC_base
{
    ...
    function isOpped($channel, $nickname = null)
    {
     ...
     /**
     * Checks if we or the given user is admined on the specified channel and returns the result.
     * ChannelSyncing is required for this.
     *
     * @see setChannelSyncing
     * @param string $channel
     ...
     * @access public
     */
    function isAdmined($channel, $nickname = null) 
    {
    
    if ($this->_channelsyncing != true) {
            $this->log(SMARTIRC_DEBUG_NOTICE, 'WARNING: isAdmined() is called and the required Channel Syncing is not activated!', __FILE__, __LINE__);
            return false;
        }
    ...
    function isOwnered($channel, $nickname = null)
    {
    
    if ($this->_channelsyncing != true) {
            $this->log(SMARTIRC_DEBUG_NOTICE, 'WARNING: isOwnered() is called and the required Channel Syncing is not activated!', __FILE__, __LINE__);
            return false;
        }
        ...
        if ($this->isJoined($channel, $nickname)) {
            if ($this->_channels[strtolower($channel)]->users[strtolower($nickname)]->owner) {
                return true;
            }
        }
    ...
    function isHalfoped($channel, $nickname = null)
    {
    
    if ($this->_channelsyncing != true) {
            $this->log(SMARTIRC_DEBUG_NOTICE, 'WARNING: isHalfoped() is called and the required Channel Syncing is not activated!', __FILE__, __LINE__);
            return false;
        }
    ...
    function loadModule($name)
    {
        ...
        if(in_array((int)$name, (int)$this->_modules)) {
        ...
        if(!in_array('access', $vars)) {
        	$this->log(SMARTIRC_DEBUG_MODULES, 'DEBUG MODULES: required variable '.$classname.'::access not found, aborting...', __FILE__, __LINE__);
        return false;
        }
    ...
    function unloadModule($name)
    {
        ...
        for ($i = 0; $i < $modulecount; $i++) {
            ...
            //print_r($module);
            //print_r($modulename);
            //print_r($this->_modules[$i]);
            if ($modulename == 'net_smartirc_module_'.$name) {
                $module->module_exit($this);
                unset($module);
                $inst = $this;
                //eval("unset(".$inst."_modules[$i]);");
                unset($inst->_modules[$i]);
    ...
    * @access public
    */
    function oper($user, $password, $priority = SMARTIRC_MEDIUM)
    {
    
    $this->_send('OPER '.$user.' '.$password.'', $priority);
    ...
    function _adduser(&$channel, &$newuser)
    {
        ...
        if ($this->isJoined($channel->name, $newuser->nick)) {
            ...
            if ($newuser->owner !== null) {
                $currentuser->owner = $newuser->owner;
            }
        ...
        if ($user->op) {
          ...
          if ($user->owner) {
            $this->log(SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: adding owner: '.$user->nick.' to channel: '.$channel->name, __FILE__, __LINE__);
            $channel->owners[$user->nick] = true;
        }
        if ($user->voice) {
                ...
                foreach ($channelkeys as $channelkey) {
                    ...
                    foreach ($channel->users as $uservalue) {
                        ...
                        if ($nick == $uservalue->nick) {
                            ...
                            if (isset($channel->owners[$nick])) {
                                // die!
                                $this->log(SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: removing him from owner list', __FILE__, __LINE__);
                                unset($channel->owners[$nick]);
                            }
...
//require_once 'SmartIRC/module.php';
