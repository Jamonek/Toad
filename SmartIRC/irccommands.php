<?php
/**
 * $Id: irccommands.php,v 1.5 2004/06/28 17:35:55 meebey Exp $
 * $Revision: 1.5 $
 * $Author: meebey $
 * $Date: 2004/06/28 17:35:55 $
 *
...
class Net_SmartIRC_irccommands extends Net_SmartIRC_base
{
    ...
    function message($type, $destination, $messagearray, $priority = SMARTIRC_MEDIUM)
    {
        ...
        switch ($type) {
            ...
            break;    
            case SMARTIRC_TYPE_SERVER_NOTICE:
            $network = str_replace(':', '', $this->_network[0][0]);
             	foreach ($messagearray as $message) {
             		$this->_send('NOTICE $'.$network.' :'.$message, $priority);
             	}
            break;
            default:
                return false;
    ...
    /**
    * ignores the hostmask of a user
    *
    * @param string $user
    * @param string $reason
    * @param integer $priority message priority, default is SMARTIRC_MEDIUM
    ...
    * @access public
    */
   /* function ignore($user, $reason = null, $priority = SMARTIRC_MEDIUM)
    {
