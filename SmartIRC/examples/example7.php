<?php
/**
 * $Id: example7.php,v 1.2.2.1 2004/12/20 15:54:51 meebey Exp $
 * $Revision: 1.2.2.1 $
 * $Author: meebey $
...
class mybot
{
    function saytime_once(&$irc)
    {
        ...
        $irc->message(SMARTIRC_TYPE_CHANNEL, '#wii-battle', '(once) the time is: '.date('H:i:s'));
        $irc->unregisterTimeid($saytime_once_id);
    }
    
    function saytime(&$irc)
    {
        $irc->message(SMARTIRC_TYPE_CHANNEL, '#wii-battle', 'the time is: '.date('H:i:s'));
    }
