<?php
/**
 * $Id: messagehandler.php,v 1.44 2007/08/05 22:57:17 meebey Exp $
 * $Revision: 1.44 $
 * $Author: meebey $
 * $Date: 2007/08/05 22:57:17 $
 *
...
class Net_SmartIRC_messagehandler extends Net_SmartIRC_irccommands
{
            ...
            foreach ($channelkeys as $channelkey) {
                ...
                foreach ($channel->users as $uservalue) {
                    ...
                    if ($ircdata->nick == $uservalue->nick) {
                        ...
                        // he was maybe op or voice or admin, update comming
                         if (isset($channel->ownerss[$ircdata->nick])) {
                            $channel->owners[$newnick] = $channel->owners[$ircdata->nick];
                            unset($channel->owners[$ircdata->nick]);
                        }
            ...
            for ($i = 0; $i < $modelength; $i++) {
                switch($mode[$i]) {
                    ...
                    case 'q':
                        ...
                        if($add) {
                            $this->log(SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: adding owner: '.$nick.' to channel: '.$channel->name, __FILE__, __LINE__);
                            $channel->owners[$nick] = true;
                            $channel->users[$lowerednick]->owner = true;
                        }
                        if ($remove) {
                            $this->log(SMARTIRC_DEBUG_CHANNELSYNCING, 'DEBUG_CHANNELSYNCING: removing owner: '.$nick.' to channel: '.$channel->name, __FILE__, __LINE__);
                            unset($channel->owners[$nick]);
                            $channel->users[$lowerednick]->owner = false;
                        }
                   break;
                    case 'a':
                        $nick = array_shift($parameters);
    ...
    function _event_notice(&$ircdata)
    {
    
    $this->_noticeevent[] = $ircdata->message;
    ...
    function _event_privmsg(&$ircdata)
    {
        if ($ircdata->type & SMARTIRC_TYPE_CTCP_REQUEST) {
            ...
            } elseif (substr($ircdata->message, 1, 6) == 'FINGER') {
             	$this->message(SMARTIRC_TYPE_CTCP_REPLY, $ircdata->nick, 'FINGER '.$this->_nick.' doesn\'t liked being fingered!!!');
           }
        }
    }
    ...
    function event_rpl_bounce(&$ircdata)
    {
    
   $this->_network[] = $ircdata->rawmessageex;
	...
	function _event_err_notexttosend(&$ircdata)
	{
	 
	 $this->log(SMARTIRC_DEBUG_NOTICE, 'DEBUG_ERROR: No text to send to server', __FILE__, __LINE__);
